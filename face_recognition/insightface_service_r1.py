...
<body>
    ...
    <script>
        ...
        function checkCameraSupport() {
            // localhost는 secure context로 간주되므로 항상 카메라 사용 가능
            if (location.hostname === 'localhost' || location.hostname === '127.0.0.1') {
                console.log('Localhost detected - camera support enabled');
                return true;
            }
...
class InsightFaceService:
    ...
    def enhance_dark_image(self, image: np.ndarray) -> np.ndarray:
        ...
        try:
            ...
            if image is None or image.size == 0:
                logger.warning("enhance_dark_image: 입력 이미지가 비어있음")
                return image
    ...
    def _evaluate_quality(self, image: np.ndarray, bbox: np.ndarray) -> float:
        ...
        if face_img.size == 0 or face_img.shape[0] == 0 or face_img.shape[1] == 0:
            logger.warning(f"얼굴 이미지가 비어있음: shape={face_img.shape}")
            return 0.0
    ...
    def _calculate_eye_aspect_ratio(self, image: np.ndarray, face) -> float:
        ...
        try:
            ...
            # 얼굴 이미지가 비어있는지 확인
            if face_img.size == 0 or face_img.shape[0] == 0 or face_img.shape[1] == 0:
                logger.warning(f"EAR 계산: 얼굴 이미지가 비어있음")
                return 0.0
    ...
    def _detect_liveness(self, image: np.ndarray, face, check_blink: bool = False, frames: List[np.ndarray] = None) -> Tuple[bool, float]:
        ...
        try:
            ...
            # 얼굴 이미지가 비어있는지 확인
            if face_img.size == 0 or face_img.shape[0] == 0 or face_img.shape[1] == 0:
                logger.warning(f"Liveness Detection: 얼굴 이미지가 비어있음")
                return False, 0.0
...
class InsightFaceAPI:
    ...
    def __init__(self, face_service: InsightFaceService):
        ...
        try:
            ...
            if hasattr(config_instance, 'IP_ACCESS_CONTROL'):
                self.access_control_config = config_instance.IP_ACCESS_CONTROL
                logger.info(f"IP 접근 제어 모드: {config_instance.IP_ACCESS_CONTROL.get('mode', 'open')}")
                if config_instance.IP_ACCESS_CONTROL.get('mode') == 'blacklist':
                    logger.info(f"차단 IP 목록: {config_instance.IP_ACCESS_CONTROL.get('blacklist_ips', [])}")
            else:
    ...
    def _register_routes(self):
        ...
        def detect_for_registration():
            ...
            try:
                ...
                # config 인스턴스에서 DEBUG_OPTIONS 확인
                if hasattr(config, 'DEBUG_OPTIONS'):
                    debug_enabled = config.DEBUG_OPTIONS.get('request_debug_enabled', False)
                    debug_folder = config.DEBUG_OPTIONS.get('debug_folder', './debug')
                ...
                if request.content_type and 'multipart/form-data' in request.content_type:
                    ...
                    if 'image' in request.files:
                        ...
                        return jsonify({
                            'success': True,  # 성공여부 (통신 정상)
                            'face_detected': False,  # 얼굴검출
                            'suitable_for_registration': False,  # 등록적합성
                            'face_encoding': [],  # 얼굴인코딩 (face_detected=false인 경우 빈 배열)
                            'age': 0,  # 나이
                            'gender': 'unknown',  # 성별
                            'message': '이미지를 선택해주세요.'  # 메시지
                        }), 200
                else:
                    # JSON 처리
                    ...
                    try:
                        ...
                        return jsonify({
                            'success': False,  # 성공여부 (통신 실패 - 잘못된 형식)
                            'face_detected': False,  # 얼굴검출
                            'suitable_for_registration': False,  # 등록적합성
                            'face_encoding': [],  # 얼굴인코딩
                            'age': 0,  # 나이
                            'gender': 'unknown',  # 성별
                            'message': '이미지 데이터를 Base64로 인코딩하여 전송해주세요.'  # 메시지
                        }), 400
                ...
                if not image_data:
                    ...
                    return jsonify({
                        ...
                        'age': 0,  # 나이
                        'gender': 'unknown',  # 성별
                        'message': '이미지 데이터가 없습니다. 다시 시도해주세요.'  # 메시지
                    }), 200
                ...
                if quality_result:
                    ...
                    if face_size_ratio < 0.15:
                        suitable_for_registration = False
                        recommendations.append('카메라에 가까이 와주세요.')
                    elif face_size_ratio > 0.7:
                        suitable_for_registration = False
                        recommendations.append('카메라에서 멀어져주세요.')
                    
                    # 얼굴 위치 체크
                    if not quality_result['position_info']['is_centered']:
                        suitable_for_registration = False
                        recommendations.append('얼굴을 중앙에 맞춰주세요.')
                    ...
                    if not pose_info['is_frontal']:
                        ...
                        if abs(pose_info['pitch']) > 15:
                            if pose_info['pitch'] > 0:
                                recommendations.append('고개를 내려주세요.')  # 위를 보고 있음
                            else:
                                recommendations.append('고개를 들어주세요.')  # 아래를 보고 있음
                        if abs(pose_info['roll']) > 10:
                            recommendations.append('고개를 바르게 해주세요.')
                        suitable_for_registration = False
                    ...
                    if quality_result['overall_score'] < 0.7:
                        suitable_for_registration = False
                        if not recommendations:  # 다른 구체적인 문제가 없으면
                            recommendations.append('더 밝은 곳에서 다시 촬영해주세요.')
                
                # 새로운 표준 응답 형식
                # 메시지 결정
                if suitable_for_registration:
                    message = '등록 가능한 얼굴입니다.'
                elif recommendations:
                    message = recommendations[0]  # 첫 번째 권장사항을 메시지로 사용
                else:
                    message = '등록에 적합하지 않은 이미지입니다.'
                
                response_data = {
                    'success': True,  # 성공여부 (통신 정상)
                    'face_detected': True,  # 얼굴검출
                    'suitable_for_registration': suitable_for_registration,  # 등록적합성
                    'face_encoding': result['embedding'] if suitable_for_registration else [],  # 얼굴인코딩 (512차원)
                    'age': result.get('age', 0),  # 나이
                    'gender': result.get('gender', 'unknown'),  # 성별
                    'message': message  # 메시지
                }
                
                # 디버깅을 위한 추가 정보 (선택적)
                # quality_result나 pose 정보는 message에 포함하거나 로깅으로 처리
                ...
                return jsonify({
                    'success': False,  # 성공여부 (처리 실패)
                    'face_detected': False,  # 얼굴검출
                    'suitable_for_registration': False,  # 등록적합성
                    'face_encoding': [],  # 얼굴인코딩
                    'age': 0,  # 나이
                    'gender': 'unknown',  # 성별
                    'message': '이미지 처리 중 오류가 발생했습니다. 다시 시도해주세요.'  # 메시지
                }), 500
