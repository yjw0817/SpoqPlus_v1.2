...
<div class="panel panel-inverse">
        <div class="card-header py-3">
            <h5 class="m-0 text-white">검색옵션</h5>
        </div>
<form name="form_search_month_sales_manage" id="form_search_month_sales_manage" method="post" action="/tsalesmain/month_sales_manage">
   ...
   <div class="bbs_search_box_a6 mb10 mt10">
		<ul>
		  <li>ㆍ 판매강사명</li>
          <li>
			 <input type="text" class="form-control" name="tnm" id="tnm"  value="<?php echo $search_val['tnm']?>" >
		  </li>
		  <li>ㆍ 회원명</li>
		  <li>
		...
		<form name="form_search_month_sales_manage" id="form_search_month_sales_manage" method="post" action="/tsalesmain/month_sales_manage">
			...
			<div class="input-group input-group-sm mb-1">
				<span class="input-group-append">
					<span class="input-group-text" style='width:120px;margin-left:5px;'>대분류 선택</span>
				</span>
				<select class="select2 form-control" style="width:222px;" name="1rd_cate_cd" id="1rd_cate_cd" onChange="cate1_ch();">
					<option value="">대분류 선택</option>
					<?php foreach($cate1 as $c1) :?>
						<option value="<?php echo $c1['1RD_CATE_CD']?>" <?php if($search_val['1rd_cate_cd'] == $c1['1RD_CATE_CD']) {?> selected <?php }?> ><?php echo $c1['CATE_NM']?></option>
					<?php endforeach;?>
				...
				<span class="input-group-append">
					<span class="input-group-text" style='width:98px;margin-left:5px;'>중분류 선택</span>
				</span>
				<select class="select2 form-control" style="width:310px;" name="2rd_cate_cd" id="2rd_cate_cd">
					<option value="">중분류 선택</option>
					<?php foreach($cate2 as $c2) :?>
						<option value="<?php echo $c2['2RD_CATE_CD']?>" <?php if($search_val['2rd_cate_cd'] == $c2['2RD_CATE_CD']) {?> selected <?php }?> ><?php echo $c2['CATE_NM']?></option>
					<?php endforeach;?>
			...
			<div class="input-group input-group-sm mb-1">
				...
				<select class=" " style="width: 150px;margin-left:5px;" name="spstat" id="spstat">
				<option value="">결제상태</option>
				<?php foreach ($sDef['PAYMT_STAT'] as $r => $v) : ?>
				<?php if($r != '') :?>
					<option value="<?php echo $r?>" <?php if($search_val['spstat'] == $r) {?> selected <?php } ?> ><?php echo $v?></option>
				...
				<select class=" " style="width: 150px;margin-left:5px;" name="spchnl" id="spchnl">
				<option value="">결제채널</option>
				<?php foreach ($sDef['PAYMT_CHNL'] as $r => $v) : ?>
				<?php if($r != '') :?>
					<option value="<?php echo $r?>" <?php if($search_val['spchnl'] == $r) {?> selected <?php } ?> ><?php echo $v?></option>
				...
				<select class=" " style="width: 150px;margin-left:5px;" name="spmthd" id="spmthd">
				<option value="">결제수단</option>
				<?php foreach ($sDef['PAYMT_MTHD'] as $r => $v) : ?>
				<?php if($r != '') :?>
					<option value="<?php echo $r?>" <?php if($search_val["spmthd"] == $r) {?> selected <?php } ?> ><?php echo $v?></option>
				...
				<select class=" " style="width: 150px;margin-left:5px;" name="spdv" id="spdv">
				<option value="">구분</option>
				<?php foreach ($sDef['SALES_DV'] as $r => $v) : ?>
				<?php if($r != '') :?>
					<option value="<?php echo $r?>" <?php if($search_val["spdv"] == $r) {?> selected <?php } ?> ><?php echo $v?></option>
				...
				<select class=" " style="width: 150px;margin-left:5px;" name="spdvr" id="spdvr">
				<option value="">사유</option>
				<?php foreach ($sDef['SALES_DV_RSON'] as $r => $v) : ?>
				<?php if($r != '') :?>
					<option value="<?php echo $r?>" <?php if($search_val["spdvr"] == $r) {?> selected <?php } ?> ><?php echo $v?></option>
...
$(function () {
    // 검색 조건 변경 시 자동 검색
    $('#1rd_cate_cd, #2rd_cate_cd, #tnm, #snm, #senm, #spstat, #spchnl, #spmthd, #spdv, #spdvr, #sdUnit, #sdcon').on('change', function() {
        performSearch();
    });
...
document.addEventListener('DOMContentLoaded', function () {
	...
	const tnmInput = document.getElementById('tnm');

    if (tnmInput) {
        tnmInput.addEventListener('keydown', function (event) {
            if (event.key === 'Enter') {
                event.preventDefault(); // 폼 자동 제출 방지 (선택 사항)
                btn_search();           // 원하는 함수 호출
            }
...
function performSearch(page = 1) {
    ...
    var formData = $('#form_search_month_sales_manage').serialize();
    formData += '&page=' + page;
    
    $.ajax({
        url: '/tsalesmain/ajax_month_sales_manage_search',
        type: 'POST',
        data: formData,
        dataType: 'json',
        beforeSend: function() {
