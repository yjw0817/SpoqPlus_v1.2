...
/* Reduce bottom spacing for tables in refund sections */
.panel-body > .table:last-child {
    margin-bottom: 0 !important;
}

/* Adjust panel-body padding for refund sections */
...
/* Radio button styles removed - only using standard refund */
		...
		<div class='row'>
			<div class="col-md-4">
				...
				<div class="panel panel-inverse">
					...
					<!-- CARD BODY [END] -->
					<!-- CARD FOOTER [START] -->
					<div class="card-footer" style="display: flex !important; justify-content: center !important; align-items: center !important; padding: 15px !important;">
						<!-- BUTTON [START] -->
                        <button type="button" class="btn btn-info btn-sm" style="margin-right: 10px;" onclick="go_info_mem('<?php echo $mem_info['MEM_SNO']?>');">회원정보로 가기</button>
						<button type="button" class="btn btn-info btn-sm" onclick="location.href='/teventmain/event_buy';">상품구매로 가기</button>
                        <!-- BUTTON [END] -->
					</div>
					<!-- CARD FOOTER [END] -->
			...
			<div class="col-md-4">
				...
				<div class="panel panel-inverse">
						...
						<table class="table table-bordered table-hover table-striped col-md-12">
							...
							<tbody>
								...
								<tr style="background-color: <?php echo $backColor ?>;">
									...
									<td>
										<select name="refund_method[<?php echo $r['PAYMT_MGMT_SNO']?>]" class="form-control form-control-sm refund-method-select" 
											data-paymt-sno="<?php echo $r['PAYMT_MGMT_SNO']?>" 
											data-paymt-amt="<?php echo $r['PAYMT_AMT']?>"
											data-paymt-mthd="<?php echo $r['PAYMT_MTHD']?>">
											<option value="">선택</option>
											<?php if($r['PAYMT_MTHD'] == "01") : // 카드 ?>
												<option value="01">카드취소</option>
												<option value="03">계좌이체</option>
					...
					<!-- CARD BODY [END] -->
					<!-- CARD FOOTER [START] -->
					<div class="card-footer" style="display: flex !important; justify-content: center !important; align-items: center !important; padding: 20px !important;">
						<!-- BUTTON [START] -->
						<!-- 일반환불/교체환불 라디오 버튼 제거 - 항상 일반환불만 사용 -->
						<input type="hidden" id="radioRefundIssue1" name="type_refund_issue" value="N" checked>
						
						<button type="button" class="btn btn-success btn-lg" style="padding: 10px 40px; font-size: 16px;" id='btn_refund_confirm' disabled="disabled">환불 완료</button>
...
// 카드 환불 처리 함수
function processCardRefunds(cardRefundList) {
	...
	// 각 카드취소 건에 대해 VAN 처리
	cardRefundList.forEach(function(item) {
		var params = "buy_event_sno=" + buy_event_sno + 
					"&sell_event_sno=" + sell_event_sno +
					"&mem_id=" + mem_id + 
					"&paymt_mgmt_sno=" + item.paymt_mgmt_sno + 
					"&paymt_amt=" + item.paymt_amt;
		...
		jQuery.ajax({
			...
			success: function (result) {
				...
				if (json_result['result'] == 'true') {
					vanSnos[item.paymt_mgmt_sno] = json_result['paymt_van_sno'];
					processedCount++;
					
					// 모든 카드취소가 완료되면 form submit
					if(processedCount == cardRefundList.length) {
						// VAN 일련번호를 form에 추가
						for(var key in vanSnos) {
							$('<input>').attr({
								type: 'hidden',
								name: 'refund_paymt_van_sno[' + key + ']',
								value: vanSnos[key]
							}).appendTo('#form_payment_submit');
						}
						
						$('#form_payment_submit').submit();
					}
				} else {
					alertToast('error','카드 환불 처리에 실패했습니다');
				}
			},
			error: function() {
				alertToast('error','카드 환불 처리 중 오류가 발생했습니다');
			}
		});
	});
}
...
// 환불 완료 버튼 활성화 조건 확인
function checkRefundComplete() {
	var allSelected = true;
	var totalRefundAmt = 0;
	
	// 환불 방법 선택 확인
	$('.refund-method-select').each(function() {
		if($(this).val() === '') {
			allSelected = false;
		} else {
			var amt = parseInt($(this).data('paymt-amt')) || 0;
			totalRefundAmt += amt;
		}
	});
	
	// 일반환불이 자동으로 선택되므로 항상 true
	var refundTypeSelected = true;
	
	// 디버깅을 위한 콘솔 출력
	console.log('allSelected:', allSelected);
	console.log('refundTypeSelected:', refundTypeSelected);
	console.log('refund method count:', $('.refund-method-select').length);
	
	// 모든 환불 방법이 선택되고 환불 유형이 선택되었을 때 활성화
	if (allSelected && refundTypeSelected) {
		$('#btn_refund_confirm').prop('disabled', false).removeAttr('disabled');
		console.log('Button enabled');
	} else {
		$('#btn_refund_confirm').prop('disabled', true).attr('disabled', 'disabled');
...
$('#btn_refund_confirm').click(function(){
	// 환불 방법 검증
	var refundMethodValid = true;
	var cardRefundList = []; // 카드취소 목록
	
	$('.refund-method-select').each(function() {
		if($(this).val() === '') {
			refundMethodValid = false;
			return false;
		} else if($(this).val() === '01') { // 카드취소
			cardRefundList.push({
				paymt_mgmt_sno: $(this).data('paymt-sno'),
				paymt_amt: $(this).data('paymt-amt'),
				paymt_mthd: $(this).data('paymt-mthd')
			});
		}
	});
	
	if(!refundMethodValid) {
		alertToast('error','모든 결제 건에 대한 환불 방법을 선택해주세요');
		return;
	}
	
	// 일반환불이 자동으로 선택되므로 검증 불필요
	...
	{
		// 카드취소 처리가 필요한 경우
		if(cardRefundList.length > 0) {
			processCardRefunds(cardRefundList);
		} else {
			// 카드 결제가 없는 경우 바로 submit
			$('#form_payment_submit').submit();
		}
	}
});
...
$(document).ready(function() {
	// 환불 방법 선택 시 환불 완료 버튼 활성화 체크
	$(document).on('change', '.refund-method-select', function() {
		checkRefundComplete();
	});

	// 일반환불/교체환불 선택 이벤트 제거 - 항상 일반환불만 사용
