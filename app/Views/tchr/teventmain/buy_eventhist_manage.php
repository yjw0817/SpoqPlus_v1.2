...
<form name="form_search_buy_event_manage" id="form_search_buy_event_manage" method="post" onsubmit="return false;">
   <div class="bbs_search_box_a mb10 mt10">
	 ...
	 <div class="bbs_search_box_a7 mb10 mt10">
		<ul>
		 <li>ㆍ 판매상품명  </li>
		  <li>
		  ...
		  <li>
			<select class="form-control me-2"  name="sdUnit" id="sdUnit" onchange="dateTypeCh(this);">
			 ...
			 <select class="form-control" name="sdcon" id="sdcon" onchange="dateConCh(this);">
				...
				<option value="ps" <?php if($search_val['sdcon'] == "ps") {?> selected <?php } ?> >결제일자</option>
...
<!-- <div class="panel panel-inverse">
	...
	<div class="card-footer clearfix">
		<form name="form_search_buy_eventhist_manage" id="form_search_buy_eventhist_manage" method="post" action="/teventmain/buy_eventhist_manage">
...
<div class="panel panel-inverse">
	<div class="panel-heading">
		<h4 class="panel-title">구매내역(결제기준)</h4>
		<div class="panel-heading-btn">
		</div>
	</div>
		<!-- CARD BODY [START] -->
	<div class="panel-body table-responsive mb20">
		<!-- TABLE [START] -->
		<!-- TABLE [START] -->
...
$(function () {
    ...
    // 선택 박스 변경시 자동 검색
    $('#spstat, #spchnl, #spmthd, #sdUnit, #sdcon').on('change', function() {
        btn_search();
    });
...
function btn_search()
{
	...
	currentSearchRequest = $.ajax({
		url: '/teventmain/ajax_buy_eventhist_search',
		type: 'POST',
		data: formData,
		dataType: 'json',
...
function highlightSearchTerm() {
    ...
    if (eventSearchTerm.length >= 2) {
        // 판매상품명에서 검색어 하이라이트
        $('.panel-body table tbody tr').each(function() {
            var $eventCell = $(this).find('td:eq(2)'); // 판매상품명 컬럼
