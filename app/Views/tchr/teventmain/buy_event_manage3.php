			...
			<div class="col_md-12">
				<div class="alert-info2" >
				  <button type="button" class="close2" onclick="$('#help_con').hide();"><i class="fas fa-times" style="font-size:20px;"></i></button>
                     <small><strong>수업남은횟수로 회원을 검색하여 보내기 할 회원을 검색합니다.</strong></small><br />
					<small>1. 검색조건 : 수업횟수로 회원을 검색합니다.</small><br />
					<small>2. 검색된 회원들에게 어떤 상품을 중분류를 선택하세요</small><br />
...
<form name="form_buy_event_manage3" id="form_buy_event_manage3" method="post" onsubmit="return false;">

   <div class="bbs_search_box_a2 mb10 mt10">
		<ul>
		  <li>ㆍ 남은수업횟수</br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;(미만)  </li>
		  <li>
			<input type="text" class="form-control" name="sch_ccnt" id="sch_ccnt"  value="<?php echo $search_val['sch_ccnt']?>" >
		  </li>
		  <li>
			 <select class="form-control" name="acc_rtrct_dv" id="acc_rtrct_dv" style="display:none;">
		   ...
		   <li style="display:none;">
			<a href="#" type="button" class="basic_bt02" onclick="$('#help_con').show();">
			<i class="far fa-question-circle"></i> 주의</a>

		  </li>	  		
		 </ul>
	</div>	
	 <div class="bbs_search_box_b2 mb10 mt10">
		<ul>
		 <li>ㆍ 분류 </li>
		  <li>
	...
	<div class="card-footer clearfix">
		<div class="row" id='help_con' style='display:none;'>
			<div class="col_md-12">
				<div class="alert alert-warning alert-dismissible" >
					...
					<small><strong>수업남은횟수로 회원을 검색하여 보내기 할 회원을 검색합니다.</strong></small><br />
					<small>1. 검색조건 : 수업횟수와 출입제한 조건으로 회원을 검색합니다. (입장불가, 수업입장)</small><br />
					<small>2. 검색된 회원들에게 어떤 상품을 중분류를 선택하세요</small><br />
					<small>3. 검색된 상품에서 상품 보내기할 회원을 선택한 해주세요. </small><br />
					<small>4. 선택이 완료되면 상품 보내기 버튼을 클릭하여 검색된 상품을 선택하여 상품을 보낼 수 있습니다.</small>
				</div>
		...
		<div class="input-group input-group-sm mb-1">
			<span class="input-group-append">
				<span class="input-group-text input-readonly" style='width:150px;margin-left:5px;'>수업남은횟수(미만)</span>
			</span>
			<input type="text" class="" name="sch_ccnt" id="sch_ccnt" style='width:100px' value="<?php echo $search_val['sch_ccnt']?>" >
...
<div class="panel panel-inverse">
	<div class="panel-heading">
		<h4 class="panel-title">보내기 상품 리스트(수업)</h4>
...
function btn_search()
{
	if ( $('#sch_ccnt').val() == '' )
	{
		alertToast('error',"수업남은횟수(미만)을 입력하세요");
		return;
	}
	...
	// AJAX 검색 요청
	var formData = $('#form_buy_event_manage3').serialize();
	
	// 로딩 표시
	$('#search-results').html('<div class="text-center"><i class="fa fa-spinner fa-spin"></i> 검색중...</div>');
	
	currentSearchRequest = $.ajax({
		url: '/teventmain/ajax_buy_event_manage3_search',
		type: 'POST',
		data: formData,
		dataType: 'json',
...
// 출입제한 조건 변경 시 자동 검색
$('#acc_rtrct_dv').change(function(){
	if ($('#sch_ccnt').val() != '') {
		btn_search();
	}
});

// 수업남은횟수 입력 후 엔터키 또는 포커스 아웃 시 자동 검색
$('#sch_ccnt').on('keyup', function(e){
...
function bindPagerEvents() {
    $('#pager-container a').off('click').on('click', function(e) {
        ...
        if (href && href !== '#') {
            ...
            // 검색 폼에 페이지 번호 추가
            $('#form_buy_event_manage3').append('<input type="hidden" name="page" value="' + page + '">');
            btn_search();
            // 페이지 번호 제거
            $('#form_buy_event_manage3 input[name="page"]').remove();
        }
    });
}
...
function restoreCheckedMembers() {
    ...
    $.ajax({
        url: '/teventmain/ajax_get_selected_mem_sno3',
        type: 'POST',
        dataType: 'json',
        success: function(response) {
