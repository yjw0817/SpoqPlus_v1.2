...
<form name="form_search_misu_manage" id="form_search_misu_manage" method="post" onsubmit="return false;">
   <div class="bbs_search_box_a mb10 mt10">
	 ...
	 <div class="bbs_search_box_a4 mb10 mt10">
		<ul>
		 ...
		 <li>
			<select class="form-control" name="srstat" id="srstat">
				<option value="">처리상태</option>
				<?php foreach ($sDef['RECVB_STAT'] as $r => $v) : ?>
				<?php if($r != '') :?>
					<option value="<?php echo $r?>" <?php if($search_val['srstat'] == $r) {?> selected <?php } ?> ><?php echo $v?></option>
				<?php endif; ?>
				<?php endforeach; ?>
				</select>
		  </li>
		...
		<ul>
		  ...
		  <li><select class="form-control me-2"  name="sdUnit" id="sdUnit" onchange="dateTypeCh(this);">
			 ...
			 <select class="form-control" name="sdcon" id="sdcon" onchange="dateConCh(this);">
				...
				<option value="ms" <?php if($search_val['sdcon'] == "ms") {?> selected <?php } ?> >처리일시</option>
				<!-- <option value="pr" <?php if($search_val['sdcon'] == "pr") {?> selected <?php } ?> >환불일자</option> -->
			</select>
		  </li>
		   <li style="width:36px">
		  ...
		  <li style="width:auto">
			...
			<input type="text" class="form-control me-2" style="text-align:center; width:100px; <?php if($search_val['sdUnit'] != 'Week' && $search_val['sdUnit'] != 'Month') {?> display:none; <?php } ?>" name="nthUnit" id="nthUnit" value="<?php echo $search_val['nthUnit']?>" autocomplete='off'  readonly >
...
<!-- <div class="panel panel-inverse">
	...
	<div class="card-footer clearfix">
		<form name="form_search_misu_manage" id="form_search_misu_manage" method="post" action="/teventmain/misu_manage">
			<div class="input-group input-group-sm mb-1">
				...
				<select class=" " style="width: 150px;margin-left:5px;" name="srstat" id="srstat">
				<option value="">처리상태</option>
				<?php foreach ($sDef['RECVB_STAT'] as $r => $v) : ?>
				<?php if($r != '') :?>
					<option value="<?php echo $r?>" <?php if($search_val['srstat'] == $r) {?> selected <?php } ?> ><?php echo $v?></option>
...
<div class="panel panel-inverse">
	<div class="panel-heading">
		<h4 class="panel-title">미수금 현황</h4>
	...
	<div class="panel-body table-responsive">
		<!-- TABLE [START] -->
		<!-- TABLE [START] -->
		총 건수 : <?php echo $totalCount?> 건 / 총 미수금 결제금액 : <?php echo number_format($sumCost)?> 원
		<span style="font-size:0.8rem;color:red;">
		( 등록 : 첫 결재시 등록된 미수금 상태
		/ 처리 : 미수금을 처리 하였으나 남은 미수금이 있는 상태
		/ 완료 : 미수금을 모두 처리한 상태 )
		</span>
		<table class="table table-bordered table-hover col-md-12 mt20">
			...
			<tbody>
				...
				<tr style="background-color: <?php echo $backColor ?>;">
					...
					<td style="text-align:right"><?php echo number_format($r['RECVB_PAYMT_AMT'])?></td>
					<td style="text-align:right"><?php echo number_format($r['RECVB_LEFT_AMT'])?></td>
					<td class='text-center'><?php echo $sDef['RECVB_STAT'][$r['RECVB_STAT']]?></td>
					<td><?php echo $r['MOD_DATETM']?></td>
				</tr>
				<?php 
...
$(function () {
    ...
    // 선택 박스 변경시 자동 검색
    $('#srstat, #sdUnit, #sdcon').on('change', function() {
        btn_search();
    });
	...
	currentSearchRequest = $.ajax({
		...
		success: function(response) {
			if (response.result == 'true') {
				...
				}).first().replaceWith('총 건수 : ' + response.totalCount + ' 건 / 총 미수금 결제금액 : ' + response.sumCost + ' 원');
...
function bindPagerEvents() {
    $('.card-footer a').off('click').on('click', function(e) {
        ...
        if (href && href !== '#') {
            ...
            // 검색 폼에 페이지 번호 추가
            $('#form_search_misu_manage').append('<input type="hidden" name="page" value="' + page + '">');
            btn_search();
            // 페이지 번호 제거
            $('#form_search_misu_manage input[name="page"]').remove();
        }
    });
}
...
function highlightSearchTerm() {
    ...
    if (eventSearchTerm.length >= 2) {
        // 상품명에서 검색어 하이라이트
        $('.panel-body table tbody tr').each(function() {
            var $eventCell = $(this).find('td:eq(2)'); // 상품명 컬럼
