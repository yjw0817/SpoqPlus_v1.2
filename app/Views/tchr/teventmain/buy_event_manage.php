...
<form name="form_search_buy_event_manage" id="form_search_buy_event_manage" method="post" onsubmit="return false;">
		...
		<ul>
		 <li>ㆍ 판매상품명  </li>
		  <li>
			...
			<li>
				<select class="form-control"    name="sestat" id="sestat">
				<option value="">상태</option>
				<?php foreach ($sDef['EVENT_STAT'] as $r => $v) : ?>
				<?php if($r != '') :?>
					<option value="<?php echo $r?>" <?php if($search_val['sestat'] == $r) {?> selected <?php } ?> ><?php echo $v?></option>
				<?php endif; ?>
				<?php endforeach; ?>
				</select>
          ...
          <li>
			 <select class="form-control"  name="sestatr" id="sestatr">
				<option value="">판매상태</option>
				<?php foreach ($sDef['EVENT_STAT_RSON'] as $r => $v) : ?>
				<?php if($r != '') :?>
					<option value="<?php echo $r?>" <?php if($search_val['sestatr'] == $r) {?> selected <?php } ?> ><?php echo $v?></option>
				<?php endif; ?>
				<?php endforeach; ?>
				</select>
		  </li>		  		  
	 ...
	 <div class="bbs_search_box_a4 mb10">
		<ul>
		  ...
		  <li>
			<select class="form-control me-2"  name="sdUnit" id="sdUnit" onchange="dateTypeCh(this);">
			 ...
			 <select class="form-control"  name="sdcon" id="sdcon" onchange="dateConCh(this);">
				...
				<option value="exs" <?php if($search_val['sdcon'] == "exs") {?> selected <?php } ?> >시작일</option>
				<option value="exe" <?php if($search_val['sdcon'] == "exe") {?> selected <?php } ?> >종료일</option>
				<option value="exb" <?php if($search_val['sdcon'] == "exb") {?> selected <?php } ?> >구매일</option>
			</select>
		  ...
		  <li style="width:auto">
			...
			<input type="text" class="form-control me-2" style="text-align:center; <?php if($search_val['sdUnit'] != 'Week' && $search_val['sdUnit'] != 'Month') {?> display:none; <?php } ?>" name="nthUnit" id="nthUnit" value="<?php echo $search_val['nthUnit']?>" autocomplete='off'  readonly >
...
<!-- <div class="panel panel-inverse">
	...
	<div class="card-footer clearfix">
		<form name="form_search_buy_event_manage" id="form_search_buy_event_manage" method="post" action="/teventmain/buy_event_manage">
			<div class="input-group input-group-sm mb-1">
				...
				<select class=" " style="width: 150px;margin-left:5px;" name="sestat" id="sestat">
				<option value="">상태</option>
				<?php foreach ($sDef['EVENT_STAT'] as $r => $v) : ?>
				<?php if($r != '') :?>
					<option value="<?php echo $r?>" <?php if($search_val['sestat'] == $r) {?> selected <?php } ?> ><?php echo $v?></option>
				...
				<select class=" " style="width: 150px;margin-left:5px;" name="sestatr" id="sestatr">
				<option value="">판매상태</option>
				<?php foreach ($sDef['EVENT_STAT_RSON'] as $r => $v) : ?>
				<?php if($r != '') :?>
					<option value="<?php echo $r?>" <?php if($search_val['sestatr'] == $r) {?> selected <?php } ?> ><?php echo $v?></option>
...
<div class="panel panel-inverse">
	<div class="panel-heading">
		<h4 class="panel-title">구매내역(판매기준)</h4>
	...
	<div class="panel-body table-responsive">
		<!-- TABLE [START] -->
		<!-- TABLE [START] -->
		총 건수 : <?php echo $totalCount?> 건 <br />
		총 판매금액 : <?php echo number_format($sumCost1)?> 원
		/ 총 결제금액 : <?php echo number_format($sumCost2)?> 원
		/ 총 미수금액 : <?php echo number_format($sumCost3)?> 원
		<table class="table table-bordered table-hover col-md-12 mt20">
			...
			<tbody>
				...
				<tr style="background-color: <?php echo $backColor ?>;">
					...
					<td><?php echo $sDef['EVENT_STAT'][$r['EVENT_STAT']]?></td>
					...
					<td><span id="<?php echo "exr_s_date_".$r['BUY_EVENT_SNO']?>"><?php echo $r['EXR_S_DATE']?></span></td>
...
$(function () {
    ...
    // 선택 박스 변경시 자동 검색
    $('#sestat, #sestatr, #sdUnit, #sdcon').on('change', function() {
        btn_search();
    });
...
function btn_search()
{
	...
	// AJAX 검색 요청
	var formData = $('#form_search_buy_event_manage').serialize();
	
	// 로딩 표시
	$('.panel-body table tbody').html('<tr><td colspan="17" class="text-center"><i class="fa fa-spinner fa-spin"></i> 검색중...</td></tr>');
	
	currentSearchRequest = $.ajax({
		url: '/teventmain/ajax_buy_event_search',
		type: 'POST',
		data: formData,
		dataType: 'json',
		success: function(response) {
			if (response.result == 'true') {
				...
				$('.panel-body').contents().filter(function() {
					return this.nodeType === 3 && this.textContent.includes('총 판매금액');
				}).first().replaceWith('총 판매금액 : ' + response.sumCost1 + ' 원 / 총 결제금액 : ' + response.sumCost2 + ' 원 / 총 미수금액 : ' + response.sumCost3 + ' 원');
		...
		error: function(xhr) {
			...
			if (xhr.statusText !== 'abort') {
				alertToast('error', '검색 중 오류가 발생했습니다.');
				$('.panel-body table tbody').html('<tr><td colspan="17" class="text-center">검색 중 오류가 발생했습니다.</td></tr>');
			}
		},
		complete: function() {
...
function bindPagerEvents() {
    $('.card-footer a').off('click').on('click', function(e) {
        ...
        if (href && href !== '#') {
            ...
            // 검색 폼에 페이지 번호 추가
            $('#form_search_buy_event_manage').append('<input type="hidden" name="page" value="' + page + '">');
            btn_search();
            // 페이지 번호 제거
            $('#form_search_buy_event_manage input[name="page"]').remove();
        }
    });
}
