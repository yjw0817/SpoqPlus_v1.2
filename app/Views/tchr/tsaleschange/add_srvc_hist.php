...
<form name="form_search_add_srvc_hist" id="form_search_add_srvc_hist" method="post" onsubmit="return false;">
		...
		<ul>
		  ...
		  <li>
			 <select class="form-control" name="sdcon" id="sdcon" onchange="dateConCh_D(this);">
				...
				<option value="sdd" <?php if($search_val['sdcon'] == "sdd") {?> selected <?php } ?> >강제종료일</option>
			</select>
...
<div class="panel panel-inverse">
	...
	<div class="card-footer clearfix">
		<form name="form_search_add_srvc_hist" id="form_search_add_srvc_hist" method="post" action="/tsaleschange/add_srvc_hist">
...
<div class="panel panel-inverse">
	<div class="panel-heading">
		<h3 class="panel-title">수업추가 내역 리스트</h3>
	</div>
		<!-- CARD BODY [START] -->
		...
		<table class="table table-bordered table-hover table-striped col-md-12 mt20">
			<thead>
				<tr class='text-center'>
					...
					<th>판매상품명</th>
					<th style='width:180px'>추가 수업수</th>
					
					<th style='width:180px'>수업 진행 강사</th>
			...
			<tbody>
				...
				<tr style="background-color: <?php echo $backColor ?>;">
					...
					<td><?php echo $r['ADD_SRVC_CLAS_CNT']?></td>
					<td><?php echo $r['TCHR_NM']?></td>
...
function btn_search()
{
	...
	// AJAX 검색 요청
	var formData = $('#form_search_add_srvc_hist').serialize();
	
	// 로딩 표시
	$('#search-results').html('<div class="text-center"><i class="fa fa-spinner fa-spin"></i> 검색중...</div>');
	
	currentSearchRequest = $.ajax({
		url: '/tsaleschange/ajax_add_srvc_hist_search',
		type: 'POST',
		data: formData,
		dataType: 'json',
...
function bindPagerEvents() {
    $('#pager-container a').off('click').on('click', function(e) {
        ...
        if (href && href !== '#') {
            ...
            // 검색 폼에 페이지 번호 추가
            $('#form_search_add_srvc_hist').append('<input type="hidden" name="page" value="' + page + '">');
            btn_search();
            // 페이지 번호 제거
            $('#form_search_add_srvc_hist input[name="page"]').remove();
        }
    });
}
