...
<form name="form_search_tchr_change_hist" id="form_search_tchr_change_hist" method="post" onsubmit="return false;">
		...
		<ul>
		  ...
		  <li>
			 <select class="form-control"   name="chtp" id="chtp">
					...
					<option value="CHTS" <?php if($search_val['chtp'] == 'CHTS') {?> selected <?php }?> >수업강사변경</option>
					<option value="CHTP" <?php if($search_val['chtp'] == 'CHTP') {?> selected <?php }?> >판매강사변경</option>
			 </select>
		  </li>
		            <!-- 검색 버튼 제거 - 자동 검색으로 대체 -->
		 </ul>
		...
		<ul>
		  ...
		  <li>
			 <select class="form-control" name="sdcon" id="sdcon" onchange="dateConCh_D(this);">
				...
				<option value="tch" <?php if($search_val['sdcon'] == "tch") {?> selected <?php } ?> >변경일</option>
			</select>
		  </li>
		  <li>
...
<!-- <div class="panel panel-inverse">
	...
	<div class="card-footer clearfix">
		<form name="form_search_tchr_change_hist" id="form_search_tchr_change_hist" method="post" action="/tsaleschange/tchr_change_hist">
...
<div class="panel panel-inverse">
	<div class="panel-heading">
		<h3 class="panel-title">강사변경 내역 리스트</h3>
	</div>
		<!-- CARD BODY [START] -->
		...
		<table class="table table-bordered table-hover table-striped col-md-12 mt20">
			<thead>
				<tr class='text-center'>
					...
					<th>판매상품명</th>
					<th style='width:130px'>변경전 강사</th>
					<th style='width:130px'>변경후 강사</th>
			...
			<tbody>
				...
				<tr style="background-color: <?php echo $backColor ?>;">
					...
					<td><?php echo $r['BF_TCHR_NM']?></td>
					<td><?php echo $r['AF_TCHR_NM']?></td>
...
function btn_search()
{
	...
	// AJAX 검색 요청
	var formData = $('#form_search_tchr_change_hist').serialize();
	
	// 로딩 표시
	$('#search-results').html('<div class="text-center"><i class="fa fa-spinner fa-spin"></i> 검색중...</div>');
	
	currentSearchRequest = $.ajax({
		url: '/tsaleschange/ajax_tchr_change_hist_search',
		type: 'POST',
		data: formData,
		dataType: 'json',
...
function bindPagerEvents() {
    $('#pager-container a').off('click').on('click', function(e) {
        ...
        if (href && href !== '#') {
            ...
            // 검색 폼에 페이지 번호 추가
            $('#form_search_tchr_change_hist').append('<input type="hidden" name="page" value="' + page + '">');
            btn_search();
            // 페이지 번호 제거
            $('#form_search_tchr_change_hist input[name="page"]').remove();
        }
    });
}
