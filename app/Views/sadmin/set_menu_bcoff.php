			...
			<div class="panel panel-inverse">
				...
				<div class="panel-body">
					<div class="row mb-2">
						...
						<select class="form-select w-25" id="compCd" disabled>
						<option value="">전체</option>
						<?php foreach ($company_list as $r) : ?>
							<?php if($r != '') : ?>
								<option value="<?php echo $r['COMP_CD']; ?>" 
									<?php echo ($r['COMP_CD'] == $comp_cd) ? 'selected' : ''; ?>>
									<?php echo $r['COMP_NM']; ?>
								</option>
							<?php endif; ?>
						<?php endforeach; ?>
...
<script>
	...
	function fnGridListBind(jsonData){
        oTable = $('#grid').DataTable({
		    ...
		    "aoColumns":[  
				 { "sTitle": "지점명", "mDataProp": "BCOFF_NM", "sWidth": "10%", "sClass": "cen", "bSortable": false }
    		 	, { "sTitle": "지점코드", "mDataProp": "BCOFF_CD", "sWidth": "5%", "sClass": "cen", "bSortable": false }
				, { "sTitle": "지점관리자명", "mDataProp": "MNGR_NM", "sWidth": "5%", "sClass": "cen", "bSortable": false }
				, { "sTitle": "연락번호", "mDataProp": "MNGR_TELNO", "sWidth": "5%", "sClass": "cen", "bSortable": false,
					"render": function (oObj) {
                        let phone = oObj.toString(); // 숫자를 문자열로 변환
						if (/^010\d{8}$/.test(phone)) { // 010으로 시작하고 총 11자리 숫자인 경우
							return phone.replace(/(\d{3})(\d{4})(\d{4})/, '$1-$2-$3');
						}
						return phone; // 전화번호가 아니면 그대로 반환
                    }
				 }
	...
	function fnMenuTreeList(compCd, bcoffCd){
		...
		$.ajax({
			type: 'POST',
			url: '/smgrmain/getMenuTreeListAtBcoff',
    		dataType:'json',
    		data:JSON.stringify(tblParam),
    		contentType:'application/json; charset=utf-8',
			async : false,
	...
	function fnSaveBcoffMenu() {
		...
		var url = '/smgrmain/saveBcoffMenu';
