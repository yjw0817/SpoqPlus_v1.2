...
?>
<!-- Main content -->
 <div class="new-title">PT 수업진행 금액</div>
<section class="content">
        ...
        <div class="row" id='status-info'>
            <div class="col-lg-3 col-6">
                ...
                <h3><?php echo number_format($topinfo['get1'])?><sup style="font-size: 20px"> 원</sup></h3>
                <p>당월 수업진행 금액</p>
                </div>
            ...
            <div class="col-lg-3 col-6">
                ...
                <div class="inner">
                <h3><?php echo number_format($topinfo['get2'])?><sup style="font-size: 20px"> 회</sup></h3>
                <p>당일 수업진행 횟수</p>
                </div>
                        ...
                        <ul class="products-list product-list-in-card pl-2 pr-2">
                            ...
                            <li class="item">
                                ...
                                <a href="javascript:void(0)" class="product-title" style='color:<?php echo $item_color?>'>
                                	<?php echo $r['SELL_EVENT_NM']?> <?php echo $item_word?><?php if(!empty($r['CLAS_MGMT_SNO'])) : ?><span class="badge badge-warning">수업체크됨</span><?php endif; ?>
                                ...
                                <span class="product-description">
                                	...
                                	<span style='float:right'>
                                    	<button type="button" class="btn btn-xs" onclick="pt_chk('<?php echo $r['BUY_EVENT_SNO']?>','<?php echo $r['MEM_SNO']?>');"><i class="fas fa-chevron-right"></i></button>
                                    </span>
                                </span>
    ...
    <div class="row">
    	<div class="col_md-12" style='width:100%'>
    		<div class="" id="bottom-menu-area">
                <div style="padding: 5px 10px 0 10px; position: relative; height: 30px;">
                    <button type="button" class="close" id="bottom-menu-close" style="position: absolute; top: 10px; right: 15px;">&times;</button>
                </div>
                <div id='bottom-content'>
					
					<div class="card-body" style="position: absolute; top: 30px; left: 0; right: 0; bottom: 0; display: flex; flex-direction: column; padding: 0;">
						...
						<div style="padding: 10px; flex-shrink: 0;">
							...
							<button type="button" class="btn btn-warning btn-sm col-12" id="btn_clas_cancel" style="margin-bottom:7px;" disabled>수업체크 취소</button>
						</div>
						<input type='hidden' name='buy_sno' id='pt_chk_buy_sno' />
						<input type='hidden' name='mem_sno' id='pt_chk_mem_sno' />

                        <div class="direct-chat-messages" id="clas_msg" style="flex: 1; overflow-y: auto; margin-bottom: 10px; max-height: calc(100% - 148px);">
...
<div class="overlay2">
    <div class="row"><div class="new-title">PT 수업진행 금액 검색</div>
    	<div class="col_md-12" style='width:100%'>
    		<div class="" id="bottom-menu-area">
    			<form name="form_tmem_payment" id="form_tmem_payment" method="post" action="/api/tmem_attdmemlist">
                    ...
                    <div class='bottom-content'>
                        ...
                        <button type="button" class='btn btn-block bga-main ft-white btn-success btn-sm p-3 bottom-menu' onclick="btn_search();">검색하기</button>
...
function pt_chk(buy_sno, mem_sno)
{
	...
	$('.overlay2').hide();
 	...
 	var today = new Date();
	var attd_ymd = today.getFullYear() + String(today.getMonth() + 1).padStart(2, '0') + String(today.getDate()).padStart(2, '0');
 	var params = "buy_sno="+buy_sno+"&attd_ymd="+ attd_ymd+"&mem_sno="+mem_sno;
    jQuery.ajax({
...
cmsg += "</div>";					
				...
				// 마지막 메시지 시간 업데이트 (가장 최신 메시지 시간)
				if (json_result['msg_list'].length > 0) {
					// ASC로 정렬되어 있으므로 마지막 메시지가 가장 최신
					var lastMsg = json_result['msg_list'][json_result['msg_list'].length - 1];
					lastMessageTime = lastMsg['CRE_DATETM'];
				}
				
				// 맨 아래로 스크롤
				$('.direct-chat-messages').scrollTop($('.direct-chat-messages')[0].scrollHeight);
				
				// 실시간 업데이트를 위한 인터벌 설정 (5초)
...
$("#btn_clas_comment").click(function(){
    ...
    jQuery.ajax({
        ...
        success: function (result) {
			...
			{
				$('#clas_conts').val('');
				pt_chk($('#pt_chk_buy_sno').val(), $('#pt_chk_mem_sno').val());
			}
        }
    }).done((res) => {
    	// 통신 성공시
