...
<section class="content">
	<div class="container-fluid">
		...
		<!-- CARD HEADER [END] -->
		<?php
		if (count($sarly_mgmt) > 0 ) :
		foreach ($sarly_mgmt as $r) :
		?>
		<div class="row">
			<div class="col-md-12">
                ...
                <div class="card card-primary">
					...
					<div class="card-body card-table">
						...
						<table class="table table-bordered table-hover col-md-12">
        					<thead>
        						<tr class='text-center bg-dark'>
        							<th colspan="4"><?php echo $sDef['SARLY_PMT_MTHD_NAME'][$r['SARLY_PMT_MTHD']]?></th>
        						</tr>
                        ...
                        <table class="mt20 table table-bordered table-hover col-md-12">
        					<thead>
        					...
        					<tbody>
        						<tr class='text-center'>
        							<?php if ( $r['sell_amt'] != 0 ) : ?>
        							<td onclick="detail_info('<?php echo $ss_yy?>','<?php echo $ss_mm?>','<?php echo $_SESSION['user_id']?>','01','<?php echo $r['SARLY_MGMT_SNO']?>');"><?php echo number_format($r['sell_amt'])?></td>
        							<?php else :?>
        							<td style='color:red'><?php echo number_format($r['sell_amt'])?></td>
        							<?php endif;?>
        							
        							<?php if ( $r['pt_amt'] != 0 ) : ?>
        							<td onclick="detail_info('<?php echo $ss_yy?>','<?php echo $ss_mm?>','<?php echo $_SESSION['user_id']?>','02','<?php echo $r['SARLY_MGMT_SNO']?>');"><?php echo number_format($r['pt_amt'])?></td>
        							<?php else :?>
        							<td style='color:red'><?php echo number_format($r['pt_amt'])?></td>
        							<?php endif;?>
        							
        							<?php if ( $r['pt_count'] != 0 ) : ?>
        							<?php if($r['SARLY_PMT_MTHD'] == '32') :?>
        							<td onclick="detail_info('<?php echo $ss_yy?>','<?php echo $ss_mm?>','<?php echo $_SESSION['user_id']?>','04','<?php echo $r['SARLY_MGMT_SNO']?>');"><?php echo number_format($r['pt_count'])?></td>
        							<?php else :?>
        							<td onclick="detail_info('<?php echo $ss_yy?>','<?php echo $ss_mm?>','<?php echo $_SESSION['user_id']?>','03','<?php echo $r['SARLY_MGMT_SNO']?>');"><?php echo number_format($r['pt_count'])?></td>
        							<?php endif;?>
        							<?php else :?>
        							<td style='color:red'><?php echo number_format($r['pt_count'])?></td>
        							<?php endif;?>
        						</tr>
        					</tbody>
        					...
        					<tbody>
        						<tr class='text-center'>
        							<td><?php echo number_format($r['calcu_set_rate'])?></td>
        							<td><?php echo number_format($r['calcu_set_amt'])?></td>
        							<td><?php echo number_format($r['calcu_set_1tm'])?></td>
        						</tr>
        					</tbody>
        					<thead>
        						<tr class='text-center' style='background-color:#dee2e6'>
        							<th colspan="4" class='text-center'>합계 : <?php echo number_format($r['cost'])?> 원</th>
        						</tr>
        					</thead>
                        </table>
			...
			<div class="col-md-12">
                <div class="card card-primary">
                	<div class="card-body text-center" style='height:300px;'>
                		<div style='font-size:0.9rem;margin-top:110px;'>수당집계 내용이 없습니다.</div>
                	</div>
                </div>
            </div>
        </div>
...
<!-- ============================= [ modal-default START ] ======================================= -->	
<div class="overlay">
    <div class="row"><div class="new-title">수당집계 검색</div>
    	<div class="col_md-12" style='width:100%'>
    		<div class="" id="bottom-menu-area">
    			<form name="form_tsalary" id="form_tsalary">
...
function detail_info(ss_yy,ss_mm,mid,kind,sno)
{
	location.href="/api/tsalary_detail/"+ss_yy+"/"+ss_mm+"/"+mid+"/"+kind+"/"+sno;
}
...
function btn_search()
{
	var yy = $('#ss_yy').val();
	var mm = $('#ss_mm').val();
	location.href="/api/tsalary/"+yy+"/"+mm;
}
