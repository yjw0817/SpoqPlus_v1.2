...
// 점이 구역 내부에 있는지 확인하는 함수
function isPointInZone(point, zoneObject) {
    if (!zoneObject || !zoneObject._objects || zoneObject._objects.length < 1) {
        return false;
    }
    
    const polygon = zoneObject._objects[0];
    if (!(polygon instanceof fabric.Polygon)) {
        return false;
    }
    
    // 구역 그룹의 변환 행렬 가져오기
    const matrix = zoneObject.calcTransformMatrix();
    
    // 폴리곤의 점들을 실제 캔버스 좌표로 변환
...
// 락커 그룹을 구역 경계 내로 제한하는 함수
function constrainLockerGroupToZone(lockerGroup, zoneObject) {
    if (!zoneObject || !lockerGroup) return;
    
    const zone = zoneObject._objects[0];
    if (!(zone instanceof fabric.Polygon)) return;
    
    // 구역의 경계 상자 계산
    const zoneMatrix = zoneObject.calcTransformMatrix();
    const zonePoints = zone.points.map(p => 
        fabric.util.transformPoint({ x: p.x, y: p.y }, zoneMatrix)
    );
    ...
    // 락커 그룹의 경계 상자
    const groupBounds = lockerGroup.getBoundingRect();
    
    // 위치 조정
    let newLeft = lockerGroup.left;
    let newTop = lockerGroup.top;
    
    // 왼쪽 경계 체크
    if (groupBounds.left < minX) {
        newLeft += (minX - groupBounds.left);
    }
    // 오른쪽 경계 체크
    if (groupBounds.left + groupBounds.width > maxX) {
        newLeft -= (groupBounds.left + groupBounds.width - maxX);
    }
    // 위쪽 경계 체크
    if (groupBounds.top < minY) {
        newTop += (minY - groupBounds.top);
    }
    // 아래쪽 경계 체크
    if (groupBounds.top + groupBounds.height > maxY) {
        newTop -= (groupBounds.top + groupBounds.height - maxY);
    }
    
    // 크기 제한 (스케일링 시)
    if (lockerGroup.__isScaling) {
        const maxScaleX = (maxX - minX) / lockerGroup.width;
        const maxScaleY = (maxY - minY) / lockerGroup.height;
        
        if (lockerGroup.scaleX > maxScaleX) {
            lockerGroup.scaleX = maxScaleX;
        }
        if (lockerGroup.scaleY > maxScaleY) {
            lockerGroup.scaleY = maxScaleY;
        }
    }
    
    // 위치 업데이트
    if (newLeft !== lockerGroup.left || newTop !== lockerGroup.top) {
        lockerGroup.set({
            left: newLeft,
            top: newTop
        });
        lockerGroup.setCoords();
    }
    
    // 캔버스 다시 렌더링
    if (lockerGroup.canvas) {
        lockerGroup.canvas.renderAll();
    }
}

// 전역 함수들 (jsinc.php의 alertToast 사용)
function showToast(message, type = 'success', duration = 3000) {
    // duration이 지정된 경우 자동 닫기 Toast 사용
    if (duration && duration > 0) {
        ...
        // 자동 닫기가 있는 Toast
        const AutoToast = Swal.mixin({
            toast: true,
            position: 'top',
            showConfirmButton: false,  // 확인 버튼 제거
            timer: duration,           // 자동 닫기 설정
            width: '500px',
            footer: "Argos SpoQ",
        ...
        // duration이 없으면 기본 alertToast 사용
        alertToast(type, message);
    }
}

// 좌표 정규화 함수 (원본 이미지 기준 0-1 범위로 저장)
function normalizeCoordinates(points, canvas) {
    ...
    // 배경 이미지의 실제 표시 위치와 크기
    // Fabric.js는 기본적으로 center 기준이므로 left/top으로 변환
    const imgScaleX = bgImage.scaleX || 1;
    const imgScaleY = bgImage.scaleY || 1;
    const imgWidth = bgImage.width * imgScaleX;
    const imgHeight = bgImage.height * imgScaleY;
    
    // originX와 originY를 고려한 실제 left/top 위치 계산
    let imgLeft = bgImage.left || 0;
    let imgTop = bgImage.top || 0;
    
    // originX가 'center'인 경우 조정
    if (bgImage.originX === 'center') {
        imgLeft = imgLeft - imgWidth / 2;
    }
    
    // originY가 'center'인 경우 조정
    if (bgImage.originY === 'center') {
        imgTop = imgTop - imgHeight / 2;
    }
    
    console.log('정규화 - 배경 이미지 정보:', {
        position: { left: imgLeft, top: imgTop },
        scale: { x: imgScaleX, y: imgScaleY },
        displaySize: { width: imgWidth, height: imgHeight },
        originalSize: { width: bgImage.width, height: bgImage.height },
        origin: { x: bgImage.originX, y: bgImage.originY },
        rawPosition: { left: bgImage.left, top: bgImage.top }
    });
    
    // 캔버스 좌표를 이미지 상대 좌표(0-1)로 변환
    return points.map((point, index) => {
        ...
        if (index === 0) {
            console.log(`정규화 첫 번째 점: (${point.x}, ${point.y}) → (${relativeX}, ${relativeY})`);
            console.log(`이미지 위치: (${imgLeft}, ${imgTop}), 크기: ${imgWidth}x${imgHeight}`);
        }
...
function denormalizeCoordinates(normalizedPoints, canvas) {
    // 배경 이미지 정보 가져오기
    const bgImage = canvas.backgroundImage;
    ...
    // 좌표가 정규화되어 있는지 확인
    // 모든 좌표가 0-1.5 범위에 있고, 평균값이 작으면 정규화된 것으로 판단
    const allInNormalRange = normalizedPoints.every(point => 
        point.x >= -0.5 && point.x <= 1.5 && point.y >= -0.5 && point.y <= 1.5
    );
    ...
    console.log(`좌표 분석: 평균(${avgX}, ${avgY}), 최대(${maxX}, ${maxY}), 범위내=${allInNormalRange}`);
    
    // 정규화된 좌표 판단 기준:
    // 1. 모든 좌표가 -0.5~1.5 범위 내에 있고
    // 2. 최대값이 2보다 작으면 정규화된 좌표로 판단
    const isNormalized = allInNormalRange && maxX < 2 && maxY < 2;
    
    if (!isNormalized) {
        console.log('이미 캔버스 좌표입니다. 변환 건너뜀.');
        return normalizedPoints;
    }
    
    console.log('정규화된 좌표로 판단됨. 캔버스 좌표로 변환 진행.');
    ...
    // 이미지 상대 좌표를 캔버스 절대 좌표로 변환
    const denormalizedPoints = normalizedPoints.map((point, index) => {
        const newX = (point.x * imgWidth) + imgLeft;
        const newY = (point.y * imgHeight) + imgTop;
        
        if (index === 0) {
            console.log(`첫 번째 점 변환: (${point.x}, ${point.y}) → (${newX}, ${newY})`);
        }
...
$(document).ready(function() {
    $('.drawing-container').each(function() {
        ...
        if (img.complete) {
            ...
            initCanvas(floorSno, imageUrl).then(() => {
                console.log('Canvas initialized for floor:', floorSno);
                // loadZones는 initCanvas 내부에서 호출됨
            });
        } else {
        ...
        // 층 삭제 버튼 이벤트
        container.find('.delete-floor').off('click').on('click', function() {
            const floorSno = $(this).data('floor');
            showConfirm('정말로 이 도면을 삭제하시겠습니까?', function() {
                $.ajax({
                    url: 'http://localhost:8080/locker/ajax_delete_floor',
                    type: 'POST',
                    data: {
                        floor_sno: floorSno,
        ...
        // 그리기 초기화 버튼 이벤트
        container.find('.clear-drawing').off('click').on('click', function() {
            const canvas = fabricCanvases[floorSno];
            if (canvas) {
                cancelDrawing(canvas);
                $(this).closest('.drawing-container').find('.draw-zone').removeClass('active');
            }
        });
        
        // 줌 초기화 버튼 이벤트
        container.find('.reset-zoom').off('click').on('click', function() {
            const canvas = fabricCanvases[floorSno];
            if (canvas && canvas.getZoom() !== 1) {
                animateZoomReset(canvas, () => {
                    console.log('Zoom reset to original size');
                });
            }
        });
    });
    ...
    $('#floorForm').on('submit', function(e) {
        ...
        $.ajax({
            url: 'http://localhost:8080/locker/ajax_upload_floor',
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
        ...
        Object.keys(fabricCanvases).forEach(floorSno => {
            ...
            if (canvas) {
                ...
                if (existingZones.length === 0) {
                    console.log('🔍 No zones found on canvas, attempting to load zones for floor:', floorSno);
                    // 캔버스가 준비되었는지 확인
                    if (canvas.backgroundImage) {
                        console.log('✅ Background image ready, loading zones immediately');
                        console.log('🖼️ Background image details:', {
                        ...
                        console.log('⏳ Background image not ready, delaying zone load by 1000ms');
                        setTimeout(() => {
                            console.log('🔄 Retry loading zones after delay...');
                            loadZones(floorSno);
                        }, 1000);
                    }
                } else {
                    console.log(`✅ Zones already loaded for floor ${floorSno}: ${existingZones.length} zones found`);
                }
            }
        });
    ...
    return new Promise((resolve, reject) => {
        try {
            ...
            const canvas = new fabric.Canvas(canvasElement, {
                ...
                backgroundColor: '#f5f5f5'  // 배경색 추가로 캔버스 영역 확인
            });

            // 이미지 로드
            fabric.Image.fromURL(imageUrl, function(img) {
                // 도면이 컨테이너에 맞도록 스케일 계산
                const scale = Math.min(
                    containerWidth / img.width,
                    containerHeight / img.height
                ) * 0.9; // 여백을 위해 0.9 배율
                
                console.log(`🎯 반응형 캔버스 설정:`);
                ...
                console.log(`   - 최종 도면 크기: ${Math.round(img.width * scale)}x${Math.round(img.height * scale)}`);
                ...
                canvas.setBackgroundImage(img, function() {
                    console.log('Background image set for floor:', floorSno);
                });
                ...
                // 이벤트 핸들러 설정
                setupCanvasEventHandlers(canvas, floorSno);
                
                // 편집 모드 버튼 이벤트 설정
                setupEditModeButton(canvas, floorSno);

                console.log('Canvas initialization complete for floor:', floorSno);
                
                // 구역 로드를 Promise로 변환하여 완료 후 렌더링
                Promise.resolve(loadZones(floorSno)).then(() => {
                    // 모든 요소가 로드된 후 렌더링 활성화
                    ...
                    console.log('All elements loaded and rendered for floor:', floorSno);
                    
                    // 디버깅: 캔버스 상태 확인
                    console.log('캔버스 객체 수:', canvas.getObjects().length);
                    console.log('캔버스 크기:', canvas.getWidth(), 'x', canvas.getHeight());
                    console.log('캔버스 줌 레벨:', canvas.getZoom());
...
function resizeCanvasArea(canvas, floorSno) {
    try {
        ...
        const oldLeft = bgImage.left || 0;
        const oldTop = bgImage.top || 0;
        ...
        // 새로운 중앙 위치 계산
        const newCenterX = (newWidth - (bgImage.width * newScale)) / 2;
        const newCenterY = (newHeight - (bgImage.height * newScale)) / 2;
        ...
        const deltaX = newCenterX - oldLeft;
        const deltaY = newCenterY - oldTop;
        
        console.log(`✅ 배경 이미지 리사이즈 완료: scale=${newScale}, center=(${newCenterX}, ${newCenterY})`);
        console.log(`변화량: scaleRatio=${scaleRatio}, delta=(${deltaX}, ${deltaY})`);
        
        // 모든 객체의 위치와 크기 업데이트
        canvas.getObjects().forEach(obj => {
            if (obj.type === 'group' && obj.data) {
                // 이전 위치에서 배경 이미지 기준 상대 위치 계산
                const relativeX = (obj.left - oldLeft) / oldScale;
                const relativeY = (obj.top - oldTop) / oldScale;
                
                // 새로운 위치 계산
                const newLeft = newCenterX + (relativeX * newScale);
                const newTop = newCenterY + (relativeY * newScale);
        ...
        console.error('캔버스 리사이즈 중 오류:', error);
    }
}
...
function loadZones(floorSno) {
    ...
    // 배경 이미지가 없으면 잠시 대기 후 재시도
    if (!canvas.backgroundImage) {
        console.log('Background image not ready, retrying in 500ms...');
        setTimeout(() => loadZones(floorSno), 500);
        return;
    }
    ...
    console.log(`Removing ${existingObjects.length} existing zone objects`);
    ...
    $.ajax({
        url: 'http://localhost:8080/locker/ajax_get_zones',
        type: 'GET',
        data: { 
            floor_sno: floorSno
        },
        ...
        beforeSend: function(xhr) {
            ...
            console.log('Canvas exists:', !!fabricCanvases[floorSno]);
            console.log('Background image exists:', !!fabricCanvases[floorSno].backgroundImage);
        },
        success: function(response) {
            ...
            console.log('🔍 Zones type:', typeof zones);
            console.log('🔍 Is zones array?', Array.isArray(zones));
            console.log('🔍 Zones length:', zones ? zones.length : 'zones is null/undefined');
            ...
            if (Array.isArray(zones) && zones.length > 0) {
                ...
                // 캔버스 상태 확인
                console.log('Canvas state after loading zones:', {
                    floor_sno: floorSno,
                    total_objects: canvas.getObjects().length,
                    zones_loaded: zones.length,
                    canvas_dimensions: {
                        width: canvas.getWidth(),
                        height: canvas.getHeight()
                    },
                    background_image: canvas.backgroundImage ? 'loaded' : 'not loaded'
                });
                ...
                console.log('❌ No zones found in response - Details:', {
                    response: response,
                    zones: zones,
                    zones_type: typeof zones,
                    zones_is_array: Array.isArray(zones),
                    zones_length: zones ? zones.length : 'N/A'
                });
                // 구역이 없어도 렌더링 복원
                canvas.renderOnAddRemove = prevRenderOnAddRemove;
            }
        ...
        error: function(xhr, status, error) {
            console.error('❌ Failed to load zones - AJAX Error');
...
// 편집 모드 버튼 설정
function setupEditModeButton(canvas, floorSno) {
    const container = $(canvas.wrapperEl).closest('.drawing-container');
    const editButton = container.find('.toggle-edit-mode');
    
    console.log('Setting up edit mode button for floor:', floorSno, 'Button found:', editButton.length);
    ...
    editButton.off('click').on('click', function(e) {
        ...
        if (!currentEditMode) {
            ...
            // 캔버스 컨테이너에 편집 모드 클래스 추가
            container.find('.canvas-container').addClass('edit-mode-active');
            
            // 모드 표시기 추가 (동적 생성)
            let modeIndicator = container.find('.mode-indicator');
            if (modeIndicator.length === 0) {
                modeIndicator = $('<div class="mode-indicator edit-mode"><i class="fas fa-edit"></i><span class="mode-text">편집 모드</span></div>');
                container.css('position', 'relative').append(modeIndicator);
            } else {
                modeIndicator.removeClass('draw-mode').addClass('edit-mode');
                modeIndicator.html('<i class="fas fa-edit"></i><span class="mode-text">편집 모드</span>');
            }
            modeIndicator.show();
...
// 캔버스 이벤트 핸들러 설정
function setupCanvasEventHandlers(canvas, floorSno) {
    let isDrawing = false;
    let currentPoints = [];
    let currentPolygon = null;
    let firstPointMarker = null;
    ...
    // 그리기 시작/취소 - 현재 캔버스의 버튼에만 이벤트 바인딩
    const drawButton = container.find('.draw-zone');
    drawButton.off('click').on('click', function() {
        ...
        // 편집 모드가 활성화되어 있으면 편집 모드를 종료하고 그리기 시작
        if (isEditMode) {
            const editButton = canvasContainer.find('.toggle-edit-mode');
            // 편집 모드를 즉시 종료 (변경사항 확인 없이)
            ...
            setTimeout(() => {
                ...
                // 캔버스 컨테이너에 그리기 모드 클래스 추가
                const container = $(canvas.wrapperEl).closest('.drawing-container');
                container.find('.canvas-container').addClass('draw-mode-active');
                
                // 모드 표시기 업데이트
                let modeIndicator = container.find('.mode-indicator');
                if (modeIndicator.length === 0) {
                    modeIndicator = $('<div class="mode-indicator draw-mode"><i class="fas fa-draw-polygon"></i><span class="mode-text">구역 그리기</span></div>');
                    container.css('position', 'relative').append(modeIndicator);
                } else {
                    modeIndicator.removeClass('edit-mode').addClass('draw-mode');
                    modeIndicator.html('<i class="fas fa-draw-polygon"></i><span class="mode-text">구역 그리기</span>');
                }
                
                console.log('Drawing mode activated for floor:', currentFloorSno);
                showToast('구역 그리기 모드가 활성화되었습니다. 캔버스를 클릭하여 구역을 그리세요.', 'info');
            }, 100);
            return;
        }
        ...
        if (isDrawing) {
            ...
            // 모드 표시기 제거
            const container = $(canvas.wrapperEl).closest('.drawing-container');
            container.find('.canvas-container').removeClass('draw-mode-active');
            container.find('.mode-indicator').removeClass('edit-mode draw-mode').hide();
            return;
        }
    ...
    canvas.on('mouse:down', function(opt) {
        ...
        if (evt.button === 0) {  // 좌클릭
            ...
            if (isDrawingLockerGroup) {
                ...
                // 현재 구역 내부인지 확인
                if (!currentZoneObject || !isPointInZone(pointer, currentZoneObject)) {
                    showToast('⚠️ 구역 내부를 클릭해주세요', 'warning', 2000);
                    return;
                }
            ...
            // canvas.getPointer()는 이미 캔버스 좌표계로 변환된 좌표를 반환
            let point = canvas.getPointer(evt);
            console.log('Mouse click point for zone drawing:', point);
    ...
    canvas.on('mouse:move', function(opt) {
        ...
        if (isDrawing && currentPoints.length > 0) {
            // canvas.getPointer()는 이미 캔버스 좌표계로 변환된 좌표를 반환
            let currentPoint = canvas.getPointer(evt);
    ...
    function showZoneSavePopup(points) {
        ...
        $('#saveZoneBtn').on('click', function() {
            ...
            console.log('NEW ZONE - Canvas coordinates:', points);
            
            // 좌표를 정규화하여 저장 (0-1 범위)
            const normalizedPoints = normalizeCoordinates(points, canvas);
            console.log('NEW ZONE - Normalized coordinates:', normalizedPoints);
            
            // 디버깅: 저장 시 좌표 정보
            console.log('💾 구역 저장 시 좌표 정보:', {
                canvas_size: { width: canvas.getWidth(), height: canvas.getHeight() },
                points_count: points.length,
                sample_point: points[0],
                normalized_sample: normalizedPoints[0]
            });
            
            // 서버에 저장 요청 (정규화된 좌표 저장)
            $.ajax({
                ...
                data: {
                    zone_nm: zoneName,
                    zone_gendr: zoneType,
                    zone_coords: JSON.stringify(normalizedPoints),  // 정규화된 좌표 저장
                ...
                success: function(response) {
                    ...
                    if (response.success || response.status === 'success') {
                        ...
                        const newZone = {
                            ...
                            zone_nm: zoneName,
                            zone_gendr: zoneType,
                            zone_coords: JSON.stringify(normalizedPoints)  // 정규화된 좌표 사용
                        };
                        ...
                        // 깜빡임 방지를 위해 renderOnAddRemove를 임시로 비활성화
                        const prevRenderOnAddRemove = canvas.renderOnAddRemove;
                        canvas.renderOnAddRemove = false;
    ...
    // X 버튼(clear-drawing) 클릭 이벤트 - 현재 캔버스의 버튼에만 이벤트 바인딩
    const clearButton = container.find('.clear-drawing');
    ...
    // 원래 크기로 되돌리기 버튼 클릭 이벤트 (애니메이션 효과) - 현재 캔버스의 버튼에만 이벤트 바인딩
    const resetZoomButton = container.find('.reset-zoom');
    resetZoomButton.off('click').on('click', function() {
...
function cancelDrawing(canvas) {
    ...
    // 그리기 버튼 비활성화
    container.find('.draw-zone').removeClass('active');
...
// 중복된 전역 이벤트 핸들러 제거됨 - setupCanvasEventHandlers 함수 내에서 개별 처리
...
function addZoneToCanvas(canvas, zone) {
    console.log('🏗️ addZoneToCanvas 시작 - Zone SNO:', zone.zone_sno, 'Zone Name:', zone.zone_nm);
    
    if (!canvas || !zone) {
        console.error('❌ Invalid parameters for addZoneToCanvas', {canvas: !!canvas, zone: !!zone});
        return;
    }

    try {
        console.log('📐 Raw zone_coords:', zone.zone_coords);
        console.log('📐 zone_coords type:', typeof zone.zone_coords);
        ...
        // 좌표가 이미 캔버스 좌표인지 확인 (denormalize 필요 여부 판단)
        // updateZoneAfterSave에서 저장된 좌표는 이미 캔버스 좌표이므로 변환 불필요
        let denormalizedCoords;
        const maxCoord = Math.max(...cleanCoords.map(p => Math.max(p.x, p.y)));
        
        if (maxCoord > 1) {
            // 이미 캔버스 좌표 (픽셀 단위)
            console.log('LOADING ZONE - Already canvas coordinates, skipping denormalization');
            denormalizedCoords = cleanCoords;
        } else {
            // 정규화된 좌표 (0-1 범위)
            console.log('LOADING ZONE - Normalized coordinates detected, denormalizing...');
            denormalizedCoords = denormalizeCoordinates(cleanCoords, canvas);
        }
        
        console.log('LOADING ZONE - Zone:', zone.zone_sno, 'Final coordinates:', denormalizedCoords);
        ...
        console.log('🔍 좌표 변환 디버깅:', {
            ...
            denormalizedCoords: denormalizedCoords,
            backgroundImage: canvas.backgroundImage ? {
                ...
                scale: {
                    x: canvas.backgroundImage.scaleX,
                    y: canvas.backgroundImage.scaleY
                },
                size: {
                    width: canvas.backgroundImage.width,
                    height: canvas.backgroundImage.height
        ...
        if (!Array.isArray(denormalizedCoords) || denormalizedCoords.length < 3) {
            console.error('❌ Invalid coordinates for zone:', zone.zone_sno, 'coords:', denormalizedCoords);
            return;
        }
        
        console.log('✅ Valid coordinates confirmed, creating polygon...');
        
        // 구역 색상 가져오기
        const color = getZoneColor(zone.zone_gendr);
        console.log('🎨 Zone color:', color);
        
        // _renderStroke 오버라이드로 인해 줌 계산 불필요
        
        // 폴리곤 생성 (denormalized 좌표 사용)
        const polygon = new fabric.Polygon(denormalizedCoords, {
            fill: 'transparent',
            stroke: color.stroke,
            strokeWidth: 2,
            strokeUniform: true, // 스케일링 시 선 두께 고정
        ...
        // 구역 이름 추가 (denormalized 좌표 사용)
        const center = getPolygonCenter(denormalizedCoords);
        ...
        console.log('🔧 Group created successfully:', {
            zone_sno: zone.zone_sno,
            zone_nm: zone.zone_nm,
            objects_in_group: group._objects.length,
            group_position: {left: group.left, top: group.top}
        });
        
        // 캔버스에 추가
        console.log('➕ Adding group to canvas...');
        canvas.add(group);
        
        console.log('✅ Zone group added to canvas:', {
            zone_sno: zone.zone_sno,
            zone_nm: zone.zone_nm,
            total_canvas_objects: canvas.getObjects().length,
            group_bounds: group.getBoundingRect(),
            group_visible: group.visible,
            polygon_visible: group._objects[0].visible,
            text_visible: group._objects[1].visible
        });
...
function setupZoneEventHandlers(group, canvas) {
    ...
    group.on('mousedown', function(e) {
        ...
        // 더블클릭 감지
        if (clickTimeout) {
            // 더블클릭 - 편집 모드가 아닐 때만 구역 확대 및 락커 그룹 관리 UI 표시
            clearTimeout(clickTimeout);
            clickTimeout = null;
            
            // 편집 모드에서는 더블클릭 무시
            if (isEditMode) {
                console.log('더블클릭 무시됨 - 편집 모드 활성화 중');
                return;
            }
...
function showZoneDetail(zoneSno) {
    // 현재 선택된 구역의 정보를 가져옴
    $.ajax({
        url: 'http://localhost:8080/locker/ajax_get_zone_detail',
        type: 'GET',
...
function updateZoneAfterSave(canvas, zone, transformedPoints) {
    try {
        ...
        // 깜빡임을 방지하기 위해 renderOnAddRemove를 임시로 비활성화
        const prevRenderOnAddRemove = canvas.renderOnAddRemove;
        canvas.renderOnAddRemove = false;
        ...
        // 좌표를 정규화
        const normalizedPoints = normalizeCoordinates(transformedPoints, canvas);
        
        // 새로운 구역 데이터 생성 - zone_coords를 JSON 문자열로 변환
        const newZoneData = {
            zone_sno: zone.data.zone_sno,
            zone_nm: zone.data.zone_nm,
            zone_gendr: zone.data.gender,
            zone_coords: JSON.stringify(normalizedPoints)  // 정규화된 좌표를 JSON 문자열로 변환
        };
        ...
        if (newZone) {
            ...
            // 원래 상태에도 업데이트
            if (originalZoneStates[canvas.floorSno] && originalZoneStates[canvas.floorSno][zone.data.zone_sno]) {
                originalZoneStates[canvas.floorSno][zone.data.zone_sno] = {
                    left: newZone.left,
                    top: newZone.top,
                    angle: newZone.angle || 0,
                    scaleX: newZone.scaleX || 1,
                    scaleY: newZone.scaleY || 1,
                    polygon_points: transformedPoints
                };
            }
        }
...
function saveZoneChanges(canvas, floorSno, button, changedZones) {
    ...
    changedZones.forEach((zoneData, index) => {
        ...
        const bgLeft = bgImage ? bgImage.left : 0;
        const bgTop = bgImage ? bgImage.top : 0;
        
        console.log('Zone transformation context:', {
            zone_sno: zone.data.zone_sno,
            zone_nm: zone.data.zone_nm,
            backgroundImage: {
                scale: bgScale,
                position: { left: bgLeft, top: bgTop }
            },
            group: {
                left: zone.left,
                top: zone.top,
        ...
        // Fabric.js 그룹의 변환 매트릭스를 사용하여 정확한 좌표 계산
        const polygon = zone._objects[0];
        const matrix = zone.calcTransformMatrix();
        
        console.log('Using transform matrix for zone:', zone.data.zone_sno);
        console.log('Transform matrix:', matrix);
        
        // 폴리곤의 원본 점들을 그룹의 변환 매트릭스로 변환
        transformedPoints = polygon.points.map((point, idx) => {
            // Fabric.js의 변환 매트릭스 적용
            const transformedPoint = fabric.util.transformPoint(
                { x: point.x, y: point.y },
                matrix
            );
            
            console.log(`Point ${idx}: (${point.x}, ${point.y}) -> (${transformedPoint.x}, ${transformedPoint.y})`);
        ...
        const canvasWidth = canvas.getWidth();
        const canvasHeight = canvas.getHeight();
        const imgWidth = bgImage ? bgImage.width * bgScale : canvasWidth;
        const imgHeight = bgImage ? bgImage.height * bgScale : canvasHeight;
        const imgRight = bgLeft + imgWidth;
        const imgBottom = bgTop + imgHeight;
        
        // 모든 점이 이미지 영역 내에 있는지 확인
        const withinImageBounds = transformedPoints.every(p => 
            p.x >= bgLeft && p.x <= imgRight && 
            p.y >= bgTop && p.y <= imgBottom
        );
        
        if (!withinImageBounds) {
            console.warn('Some points are outside image bounds. Adjusting...');
            
            // 각 점을 이미지 영역 내로 조정
            transformedPoints = transformedPoints.map(p => ({
                x: Math.round(Math.max(bgLeft, Math.min(imgRight, p.x)) * 100) / 100,
                y: Math.round(Math.max(bgTop, Math.min(imgBottom, p.y)) * 100) / 100
            }));
            
            console.log('Adjusted points:', transformedPoints);
        }
        
        console.log('MOVED ZONE - Final coordinates:', transformedPoints);
        
        // 좌표를 정규화하여 저장
        const normalizedTransformedPoints = normalizeCoordinates(transformedPoints, canvas);
        console.log('MOVED ZONE - Normalized coordinates:', normalizedTransformedPoints);
        
        $.ajax({
            url: 'http://localhost:8080/locker/ajax_save_zone',
            type: 'POST',
            data: {
                zone_sno: zoneData.zone_sno,
                zone_nm: zone.data.zone_nm,
                zone_gendr: zone.data.gender,
                zone_coords: JSON.stringify(normalizedTransformedPoints),  // 정규화된 좌표 저장
                floor_sno: floorSno,
    ...
    function checkCompletion() {
        if (savedCount + (hasError ? 1 : 0) >= totalCount) {
            if (hasError) {
                ...
                showToast('구역 변경사항이 저장되었습니다.', 'success', 2000);  // 2초 후 자동 닫기
...
function disableEditMode(canvas, floorSno, button) {
    ...
    // 캔버스 컨테이너에서 편집 모드 클래스 제거
    const container = $(canvas.wrapperEl).closest('.drawing-container');
    container.find('.canvas-container').removeClass('edit-mode-active');
...
function createLockerGroup() {
    ...
    if (lockerBoxes.length > 0) {
        ...
        // 그룹으로 만들기 - strokeWidth를 고려하여 위치 조정
        const strokeOffset = 3 / 2; // strokeWidth의 절반
        lockerGroup = new fabric.Group(lockerBoxes, {
            left: startPoint.x - strokeOffset,
            top: startPoint.y - strokeOffset,
            originX: 'left',
            originY: 'top',
            selectable: true,
            evented: true,
        ...
        // 이동 중 구역 경계 제한
        lockerGroup.on('moving', function() {
            constrainLockerGroupToZone(this, currentZoneObject);
        });
        
        // ✅ Scaling 완료 시 자동 저장
        lockerGroup.on('scaling', function() {
            // 스케일링 중임을 표시
            this.__isScaling = true;
            // 구역 경계 내로 제한
            constrainLockerGroupToZone(this, currentZoneObject);
...
function performLockerGroupDeletion(activeObject, groupName, groupSno, targetCanvas) {
    try {
        ...
        if (groupSno) {
            $.ajax({
                url: 'http://localhost:8080/locker/ajax_delete_group',
                type: 'POST',
                data: {
                    group_sno: groupSno,
                    '<?= csrf_token() ?>': '<?= csrf_hash() ?>'
...
function saveLockerGroupToDatabase(lockerGroupObject, updateData) {
    ...
    // 캔버스와 배경 이미지 정보 가져오기
    const canvas = lockerGroupObject.canvas;
    const bgInfo = canvas.backgroundImageInfo;
    
    // 배경 이미지 기준 상대 좌표로 정규화
    let normalizedLeft = 0;
    let normalizedTop = 0;
    
    if (bgInfo) {
        const imageDisplayWidth = bgInfo.originalWidth * bgInfo.scale;
        const imageDisplayHeight = bgInfo.originalHeight * bgInfo.scale;
        
        normalizedLeft = (lockerGroupObject.left - bgInfo.left) / imageDisplayWidth;
        normalizedTop = (lockerGroupObject.top - bgInfo.top) / imageDisplayHeight;
    }
    ...
    console.log(`📍 좌표 정규화: (${lockerGroupObject.left}, ${lockerGroupObject.top}) → (${normalizedLeft}, ${normalizedTop})`);
    ...
    const ajaxUrl = isNewGroup ? 
        'http://localhost:8080/locker/ajax_add_group' : 
        'http://localhost:8080/locker/ajax_update_group';
...
function loadLockerGroupsForZone(canvas, zoneSno) {
    ...
    console.log(`📊 요청 URL: http://localhost:8080/locker/get_groups`);
    console.log(`📊 요청 파라미터: zone_sno=${zoneSno}`);
    
    $.ajax({
        url: 'http://localhost:8080/locker/get_groups',
        type: 'GET',
        data: { 
            zone_sno: zoneSno
        },
...
function addLockerGroupToCanvas(canvas, groupData) {
    ...
    try {
        ...
        // 좌표가 정규화되어 있는지 확인하고 필요시 denormalize
        let finalLeft = coords.left || 100;
        let finalTop = coords.top || 100;
        
        // 좌표가 0-1 범위에 있으면 정규화된 것으로 간주
        if (coords.left >= 0 && coords.left <= 1 && coords.top >= 0 && coords.top <= 1) {
            const bgImage = canvas.backgroundImage;
            if (bgImage) {
                const imgWidth = bgImage.width * bgImage.scaleX;
                const imgHeight = bgImage.height * bgImage.scaleY;
                
                finalLeft = (coords.left * imgWidth) + bgImage.left;
                finalTop = (coords.top * imgHeight) + bgImage.top;
                console.log(`📍 정규화된 좌표를 실제 좌표로 변환: (${coords.left}, ${coords.top}) → (${finalLeft}, ${finalTop})`);
            }
        }
        ...
        // 현재 구역 객체 찾기
        const zoneObject = canvas.getObjects().find(obj => 
            obj.data && obj.data.zone_sno === groupData.zone_sno
        );
        
        // 이동 중 구역 경계 제한
        lockerGroup.on('moving', function() {
            if (zoneObject) {
                constrainLockerGroupToZone(this, zoneObject);
            }
        });
...
/* 편집 모드 활성화 시 캔버스 테두리 */
.canvas-container.edit-mode-active {
    border: 2px solid #FFC107 !important;
    box-shadow: 0 0 10px rgba(255, 193, 7, 0.3);
}

/* 그리기 모드 활성화 시 캔버스 테두리 */
.canvas-container.draw-mode-active {
...
<\!-- 락커 정면도 및 추가 기능 -->
<?php include(__DIR__ . "/locker_manage_addon.php"); ?>
