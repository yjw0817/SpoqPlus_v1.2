...
class Runner
{
    public const TASK_RETRY_NEVER = 0;
    public const TASK_RETRY_ONCE = 1;
    public const TASK_RETRY_ALWAYS = -1;

  /**
   * @var int $maxDelay The max time (in seconds) to wait before a retry.
   */
  private $maxDelay = 60;
  /**
   * @var int $delay The previous delay from which the next is calculated.
   */
  private $delay = 1;

  /**
   * @var int $factor The base number for the exponential back off.
   */
  private $factor = 2;
  /**
  ...
  /**
   * @var int $attempts The number of attempts that have been tried so far.
   */
  private $attempts = 0;
  /**
   * @var int $maxAttempts The max number of attempts allowed.
   */
   ...
   *
   * @return mixed
   * @throws GoogleServiceException on failure when no retries are available.
   */
  public function run()
  {
    while ($this->attempt()) {
